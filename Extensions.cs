...
namespace Open.Database.Extensions
{
	...
	public static partial class Extensions
	{
		...
		/// <param name="token">Optional cancellation token.</param>
		/// <returns>The result of the transform.</returns>
		public static async ValueTask<T> ExecuteReaderAsync<T>(this DbCommand command, Func<DbDataReader, T> transform, CommandBehavior behavior = CommandBehavior.Default, CancellationToken token = default)
		{
			if (transform == null) throw new ArgumentNullException(nameof(transform));
			Contract.EndContractBlock();
		...
		/// <param name="token">Optional cancellation token.</param>
		/// <returns>The result of the transform.</returns>
		public static async ValueTask<T> ExecuteReaderAsync<T>(this DbCommand command, Func<DbDataReader, ValueTask<T>> transform, CommandBehavior behavior = CommandBehavior.Default, CancellationToken token = default)
		{
			if (transform == null) throw new ArgumentNullException(nameof(transform));
			Contract.EndContractBlock();
		...
		/// <param name="predicate">The handler function that processes each IDataRecord and decides if iteration should continue.</param>
		/// <param name="token">Optional cancellation token.</param>
		public static async ValueTask IterateWhileAsync(this DbDataReader reader, Func<IDataRecord, Task<bool>> predicate, CancellationToken token = default)
		{
			if (predicate == null) throw new ArgumentNullException(nameof(predicate));
			Contract.EndContractBlock();
		...
		/// <param name="token">Optional cancellation token.</param>
		/// <param name="useReadAsync">If true (default) will iterate the results using .ReadAsync() otherwise will only Execute the reader asynchronously and then use .Read() to iterate the results but still allowing cancellation.</param>
		public static async ValueTask IterateReaderWhileAsync(this DbCommand command, Func<IDataRecord, bool> predicate, CommandBehavior behavior = CommandBehavior.Default, CancellationToken token = default, bool useReadAsync = true)
		{
			if (predicate == null) throw new ArgumentNullException(nameof(predicate));
			Contract.EndContractBlock();
		...
		/// <param name="behavior">The behavior to use with the data reader.</param>
		/// <param name="token">Optional cancellation token.</param>
		public static async ValueTask IterateReaderWhileAsync(this DbCommand command, Func<IDataRecord, Task<bool>> predicate, CommandBehavior behavior = CommandBehavior.Default, CancellationToken token = default)
		{
			if (predicate == null) throw new ArgumentNullException(nameof(predicate));
			Contract.EndContractBlock();
