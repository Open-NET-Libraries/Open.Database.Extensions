...
namespace Open.Database.Extensions
{
	...
	public static partial class ChannelExtensions
	{
			...
			CancellationToken cancellationToken = default)
		{
			...
			await target.WaitToWriteAndThrowIfClosedAsync(true, cancellationToken);
			return await command.ExecuteReader(
			...
			where T : new()
		{
			...
			await target.WaitToWriteAndThrowIfClosedAsync(true, cancellationToken);
			return await command.ExecuteReader(
				reader => reader.ToChannel(target, true, fieldMappingOverrides, cancellationToken));
		}
			...
			where T : new()
		{
			...
			return await command.ExecuteReaderAsync(reader =>
				command.UseAsyncRead && reader is DbDataReader r
				? r.ToChannelAsync(target, true, fieldMappingOverrides, cancellationToken)
				: reader.ToChannel(target, true, fieldMappingOverrides, cancellationToken));
		}
#endif
